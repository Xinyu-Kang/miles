...
def patch_qwen3_moe():
    path, src = prepare_source("/app/sglang/python/sglang/srt/models/qwen3_moe.py")
    if path is None:
        return
...
def patch_unquant_moe():
    path, src = prepare_source("/app/sglang/python/sglang/srt/layers/quantization/unquant.py")
    if path is None:
        return
...
def patch_weight_checker():
    path, src = prepare_source("/app/sglang/python/sglang/srt/utils/weight_checker.py")
    if path is None:
        return
...
#    --wandb-group qwen3-30B-A3B-async
#    --wandb-key <wandb-key>
)
